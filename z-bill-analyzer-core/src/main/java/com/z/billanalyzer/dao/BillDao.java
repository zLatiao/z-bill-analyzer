...
public class BillDao {
    public static void main(String[] args) throws SQLException, IOException {
        IBillExcelParser<WxBill, WxBillParseResult> wxBillParser = new WxBillExcelParser();
        ...
        IBillExcelParser<AlipayBill, AlipayBillParseResult> alipayBillParser = new AlipayBillExcelParser();
        ...
        IBillExcelParser<CmbBill, CmbBillParseResult> cmbBillParser = new CmbBillExcelParser();
        ...
        List<BillDetail> list = List.of(wxBillInfo, alipayBillInfo, cmbBillInfo).stream().flatMap(x -> x.getBillDetails().stream()).toList();
    ...
    // 批量写入方法（支持事务）
    public void batchInsertBills(List<BillDetail> billDetails, int batchSize) throws SQLException {
        String sql = "INSERT INTO bill (amount, amount_type, transaction_type, source, time, " +
                "counterparty, product, payment_mode, transaction_status, bill_no, " +
                "merchant_no, remark, card_number) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
